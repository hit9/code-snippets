...
// ComputeNext 为给定的模式串 p 计算 next 数组.
// next 数组的含义：
// 取字符串 p 的位置 j 之前的前缀字符串 p' ，其头尾的最大公共长度即 next[j]
// 此处采用递推方法实现
void ComputeNext(char *p, int p_length, int next[]) {
    next[0] = 0;
...
// KMP 从字符串 s 中查找字符串 p ，返回其位置.
// 找不到则返回 -1
int KMP(char *s, int s_length, char *p, int p_length) {
    // 预处理 next 数组
    int next[p_length];
    ComputeNext(p, p_length, next);
