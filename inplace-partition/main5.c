#include <stdio.h>

void Swap(int arr[], int i, int j) {
    int tmp = arr[i];
    arr[i] = arr[j];
    arr[j] = tmp;
}

// 原地分割数组，使得 <= k 的数在左边， > k 的在右边
// 返回左边的数据多少。
int PartitionInplaceStep1(int arr[], int n, int k) {
    int left = 0;
    int right = n - 1;

    while (left <= right) {
        if (arr[left] <= k) {
            left++;
        } else {
            if (left != right) Swap(arr, left, right);
            right--;
        }
    }
    return left;
}

// 原地分割数组，使得 < k 的数在左边， == k 的在右边
// 返回左边的数据多少。
void PartitionInplaceStep2(int arr[], int n, int k) {
    int left = 0;
    int right = n - 1;

    while (left <= right) {
        if (arr[left] < k) {
            left++;
        } else {
            if (left != right) Swap(arr, left, right);
            right--;
        }
    }
}

// 原地分割数组，使得 < k 的数在左边， > k 的在右边, == k 的在中间。
void PartitionInplace(int arr[], int n, int k) {
    int m = PartitionInplaceStep1(arr, n, k);
    PartitionInplaceStep2(arr, m, k);
}

int main(void) {
    int arr1[] = {7, 4, 6, 1, 2, 5, 3};
    PartitionInplace(arr1, 7, 4);
    for (int i = 0; i < 7; i++) printf("%d ", arr1[i]);
    printf("\n");

    int arr2[] = {7, 4, 6, 1, 2, 5, 3, 8};
    PartitionInplace(arr2, 8, 4);
    for (int i = 0; i < 8; i++) printf("%d ", arr2[i]);
    printf("\n");

    int arr3[] = {1, 6, 3, 2, 4, 5, 3, 2};
    PartitionInplace(arr3, 8, 3);
    for (int i = 0; i < 8; i++) printf("%d ", arr3[i]);
    printf("\n");

    int arr4[] = {7, 4, 6, 1, 1, 2, 2, 5, 5, 3, 8, 4, 4, 5};
    PartitionInplace(arr4, 14, 4);
    for (int i = 0; i < 14; i++) printf("%d ", arr4[i]);
    printf("\n");
}
